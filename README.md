<p align="center">
  <img src="docs/assets/potter-cover.png" width="1440" alt="Potter — из книги на другом языке в готовую аудиокнигу">
</p>
...
**Potter превращает этот путь в один управляемый проект — от исходного текста
до готовых глав и плейлиста.**

## Сценарий

**Сценарий закрытого продукта.** Импорт книг, модельный перевод, определение
говорящих и речевой синтез относятся к полной версии Potter. Ниже отдельно
показан запуск открытого демо на встроенной вымышленной книге.

<p align="center">
  <img src="docs/assets/book-scenario.gif" width="820" alt="Сценарий: импорт, перевод, автоматическое определение реплик и сборка аудиокниги">
</p>
...
```mermaid
flowchart LR
    A[Текстовая книга] --> B[Импорт и стабильные ID]
    B --> C[Перевод и проверка структуры]
    C --> D[Контекст книги и контроль качества]
    D --> E[Каст и сцены]
    E --> F[Независимый аудиорендер]
    F --> G[Главы, WAV и плейлист]
    H[(Checkpoint + SHA-256)] <--> B
    H <--> C
    H <--> E
    H <--> F
```
...
**Илья** · AI-инструменты для прикладных процессов · [Telegram](https://t.me/mia_ops)
